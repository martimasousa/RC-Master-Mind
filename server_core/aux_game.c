...
int validate_SNG_command(const char *cmd, char *PLID_arg, char *max_playtime_arg, const int execution_side) {
    ...
    if (!token || strcmp(token, expected_command_name) != 0) {
        fprintf(stderr, "Error: Command is malformed. It should start with %s.\n", expected_command_name);
        return ERROR;    
    }
    
    token = strtok(NULL, " ");
    if (!token || strlen(token) != PLID_DIGITS || !is_integer(token)) {
        fprintf(stderr, "Error: PLID must be an integer with 6 digits.\n");
        return ERROR;
    }
    strcpy(PLID_arg, token);

    token = strtok(NULL, " ");
    if (token[strlen(token) - 1] == '\n') token[strlen(token) - 1] = '\0';
    if (!token || strlen(token) > TIME_DIGITS || !is_integer(token)) {
        fprintf(stderr, "Error: Time must be an integer with less or equal than 3 digits.\n");
        return ERROR;
    }
    strcpy(max_playtime_arg, token);
    
    char *expected_end = (char *)cmd + strlen(expected_command_name) + 1 + strlen(PLID_arg) + 1 + strlen(max_playtime_arg) + 1;
...
int validate_DBG_command(const char *cmd, char *PLID_arg, char *max_playtime_arg, const int execution_side) {
